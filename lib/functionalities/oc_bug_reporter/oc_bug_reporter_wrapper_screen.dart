...
class _OCBugReporterWapperScreenState extends State<OCBugReporterWapperScreen> {
  ...
  bool _showBugReporter = false;
  bool _isInitialized = false;

  @override
  void initState() {
    _showBugReporter = widget.isVisible;
    super.initState();
  ...
  Future<void> _ocBugReporterSetUp() async {
    await OCBugReporterService().initService(
        key: widget.navigatorKey, listId: widget.listId, apiKey: widget.apiKey);
  ...
  Widget _bugReporterView() {
    return Positioned(
      ...
      child: Material(
        ...
        child: Visibility(
          visible: _showBugReporter && _isInitialized,
